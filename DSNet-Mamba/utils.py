...
class BceDiceLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        bceloss = self.bce(probs, target)
        diceloss = self.dice(probs, target)
...
def cal_params_flops(model, size, logger):
    """
    使用模型拷贝进行FLOPs计算，避免向原模型注入thop的hook和属性，防止训练阶段报错。
    """
    try:
        device = next(model.parameters()).device if any(p.device.type != 'meta' for p in model.parameters()) else torch.device('cpu')
        model_copy = deepcopy(model).to(device)
        model_copy.eval()
        with torch.no_grad():
            dummy = torch.randn(1, 3, size, size, device=device)
            flops, params = profile(model_copy, inputs=(dummy,))
        print('flops', flops / 1e9)
        print('params', params / 1e6)
        logger.info(f'flops: {flops/1e9}, params: {params/1e6}')
    except Exception as e:
        logger.info(f'FLOPs profiling skipped due to error: {e}')
...
def calculate_metric_percase(pred, gt, voxelspacing=None, compute_hd95=True):
    """
    计算Dice和HD95指标
    Args:
        pred: 预测结果 (binary mask)
        gt: 真实标签 (binary mask)
        voxelspacing: 体素间距，用于HD95计算。如果是3D体积，格式为(z_spacing, y_spacing, x_spacing)
                     如果是2D切片，格式为(y_spacing, x_spacing)
    """
    pred[pred > 0] = 1
    gt[gt > 0] = 1
    if pred.sum() > 0 and gt.sum()>0:
        ...
        if not compute_hd95:
            return dice, 0.0
        # HD95计算时考虑体素间距，提高准确性
        # 对于Synapse数据集，通常是3D体积，voxelspacing应该是(z_spacing, 1, 1)
        # 如果voxelspacing为None，则使用默认值(1, 1, 1)或(1, 1)
        try:
            hd95 = metric.binary.hd95(pred, gt, voxelspacing=voxelspacing)
        except Exception as e:
            # 如果计算失败（比如预测或gt为空），使用默认spacing重试
            try:
                hd95 = metric.binary.hd95(pred, gt, voxelspacing=None)
            except:
                hd95 = 0  # 如果仍然失败，返回0
        return dice, hd95
...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], 
                    test_save_path=None, case=None, z_spacing=1, val_or_test=False,compute_hd95=True):
    """
    测试单个体积数据
    Args:
        image: 输入图像
        ...
        classes: 类别数
        patch_size: patch尺寸
        test_save_path: 测试结果保存路径
        case: 病例名称
        z_spacing: z轴间距（体素间距），用于HD95计算
        val_or_test: 是否为测试模式
    """
    ...
    if len(prediction.shape) == 3:
        voxelspacing = (z_spacing, 1.0, 1.0)  # (z, y, x)
    else:
        voxelspacing = (1.0, 1.0)  # (y, x)
    
    metric_list = []
    for i in range(1, classes):
        metric_list.append(calculate_metric_percase(prediction == i, label == i, voxelspacing=voxelspacing,compute_hd95=compute_hd95))
