...
def val_one_epoch(test_datasets,
                    ...
                    config,
                    test_save_path,
                    val_or_test=False,compute_hd95=False):
    ...
    with torch.no_grad():
        ...
        for data in tqdm(test_loader):
            ...
            metric_i = test_single_volume(img, msk, model, classes=config.num_classes, patch_size=[config.input_size_h, config.input_size_w],
                                    test_save_path=test_save_path, case=case_name, z_spacing=config.z_spacing, val_or_test=val_or_test,compute_hd95=compute_hd95)
...
def test_one_epoch_synapse(test_datasets,
                    test_loader,
                    model,
                    logger,
                    config,
    ...
    with torch.no_grad():
        ...
        for data in tqdm(test_loader):
            img, msk, case_name = data['image'], data['label'], data['case_name'][0]
            metric_i = test_single_volume(img, msk, model, classes=config.num_classes, patch_size=[config.input_size_h, config.input_size_w],
                                    test_save_path=test_save_path, case=case_name, z_spacing=config.z_spacing, val_or_test=True)
            metric_list += np.array(metric_i)
        ...
        # 返回每个类别的指标
        return {
            'mean_dice': performance,
            'mean_hd95': mean_hd95,
            'class_metrics': metric_list,
            'time': etime - stime
        }