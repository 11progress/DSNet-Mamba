...
try:
    from .model_utils import (BasicBlock, Bottleneck, segmenthead, AFF, ASPP, CARAFE,
...
class DSNetMedical(nn.Module):
    ...
    Built upon DSNet with two design modifications:
      1. MSAF-M: Mamba Region Attention (MRA) replaces multi-scale region pooling in MSAF.
      2. SPASPP-M: Sequential VSSBlocks replace serial atrous convolutions in SPASPP.
    """
    ...
    def __init__(
        ...
        # MSAF-M: Mamba Region Attention in fusion module
        use_mamba_aff: bool = True,
        mamba_aff_region_size: int = 16,
        mamba_aff_drop_path: float = 0.0,
        mamba_aff_d_state: int = 16,
        mamba_aff_add_to_multiscale: bool = False,
        # SPASPP-M: VSSBlock-based context extraction
        use_mamba_spp: bool = True,
        mamba_spp_drop_path: float = 0.0,
        mamba_spp_d_state: int = 16,
        mamba_spp_depth: int = 4,
    ):
        super(DSNetMedical, self).__init__()
        self.augment = augment
        self.name = name
        self.num_classes = num_classes
        self.use_mamba_spp = use_mamba_spp
        self.use_mamba_aff = use_mamba_aff
        ...
        # MSAF-M fusion modules
        self.aff1 = Muti_AFF(
            channels=planes * 4,
            use_mamba_region=use_mamba_aff,
            mamba_region_size=mamba_aff_region_size,
            mamba_drop_path=mamba_aff_drop_path,
            mamba_d_state=mamba_aff_d_state,
            mamba_add_to_multiscale=mamba_aff_add_to_multiscale,
        )
        self.aff2 = Muti_AFF(
            channels=planes * 4,
            use_mamba_region=use_mamba_aff,
        ...
        # SPASPP-M context extraction module
        if use_mamba_spp:
            self.spp = SPASPP_VSS(
                planes * 4, planes * 4, planes * 4,
                depth=mamba_spp_depth,
                drop_path=mamba_spp_drop_path,
                d_state=mamba_spp_d_state,
            )
        else:
...
def get_dsnet_mamba(num_classes=9, dsnet_pretrained_path=None,
                    vmunet_pretrained_path=None, **kwargs):
    """Build DSNet-Mamba model (DSNet-s128 variant)."""
    model = DSNetMedical(
        m=2, n=2,
